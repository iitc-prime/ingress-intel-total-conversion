...
window.genFourColumnTable = function(blocks, centering = true) {
    var t = $.map(blocks, function(detail, index) {
        ...
        if (centering) {
            ...
            if(index % 2 === 0)
                return '<tr><th'+title+'>'+detail[0]+':</th><td'+title+'>'+detail[1]+'</td>';
            else
                return '    <th'+title+'>'+detail[0]+':</th><td'+title+'>'+detail[1]+'</td></tr>';
        }
    }).join('');
...
window.clampLatLng = function(point) {
    return turf.point([clampLng(point.geometry.coordinates[0]), clampLat(point.geometry.coordinates[1])]);
}

window.clampLatLngBounds = function(bounds) {
    return turf.bbox(turf.lineString([[clampLng(bounds[0]), clampLat(bounds[1])], [clampLng(bounds[2]), clampLat(bounds[3])]]));
}
...
window.makePermalink = function(latlng, mapView) {
    ...
    if (mapView) {
        ...
        args.push('ll='+ll2str(round([c.geometry.coordinates[1],c.geometry.coordinates[0]])), 'z='+window.map.getZoom().toFixed(6))
    }
    if (latlng) {
