...
window.MapDataRequest.prototype.start = function() {
    ...
    window.addHook('mapMoveStart', this.mapMoveStart);
    window.addHook('mapMoveEnd', this.mapMoveEnd);
...
window.MapDataRequest.prototype.mapMoveStart = function(e) {
    log.log('refresh map movestart');

    var _this = window.mapDataRequest;
    _this.setStatus('paused');
    _this.clearTimeout();
    _this.pauseRenderQueue(true);
}

window.MapDataRequest.prototype.mapMoveEnd = function(e) {
    var _this = window.mapDataRequest;
    var bounds = clampLatLngBounds(window.map.getBounds());
    var zoom = Math.floor(map.getZoom());

    if (_this.fetchedDataParams) {
        // we have fetched (or are fetching) data...
        if (_this.fetchedDataParams.mapZoom === map.getZoom()
                && turf.booleanContains(turf.bboxPolygon(_this.fetchedDataParams.bounds), turf.bboxPolygon(bounds))) {
            ...
            var remainingTime = (_this.timerExpectedTimeoutTime - new Date().getTime())/1000;

            if (remainingTime > _this.MOVE_REFRESH) {
                _this.setStatus('done','Map moved, but no data updates needed');
                _this.refreshOnTimeout(remainingTime);
                _this.pauseRenderQueue(false);
                return;
            }
        }
    }

    _this.setStatus('refreshing', undefined, -1);
    _this.refreshOnTimeout(_this.MOVE_REFRESH);
}
...
window.MapDataRequest.prototype.refresh = function() {
    ...
    var retrieve = function(west, east, south, north, tilesToFetchDistance) {
        var mapZoom = Math.floor(window.map.getZoom());
//        console.debug(west, east, south, north, mapZoom);
        var dataZoom = getDataZoomForMapZoom(mapZoom);
        var tileParams = getMapZoomTileParameters(dataZoom);

        var x1 = lngToTile(west, tileParams);
        var x2 = lngToTile(east, tileParams);
        var y1 = latToTile(north, tileParams);
        var y2 = latToTile(south, tileParams);
//        console.debug(x1, x2, y1, y2, tileParams.tilesPerEdge);
        var dataBounds = turf.bbox(turf.lineString([[tileToLng(x1  , tileParams), tileToLat(y2+1, tileParams)],
                                                    [tileToLng(x2+1, tileParams), tileToLat(y1  , tileParams)]
                                                   ]))
//        console.debug(dataBounds);

        // store the parameters used for fetching the data. used to prevent unneeded refreshes after move/zoom
        self.fetchedDataParams = { bounds: dataBounds, mapZoom: mapZoom, dataZoom: dataZoom };
        ...
        for (var y = y1; y <= y2; y++) {
            ...
            for (var x = x1; x <= x2; x++) {
                ...
                if (self.cache && self.cache.isFresh(tile_id) ) {
                    // data is fresh in the cache - just render it
                    self.pushRenderQueue(tile_id, self.cache.get(tile_id),'cache-fresh');
                    self.cachedTileCount += 1;
                } else {
                    ...
                    var tileLatLng = turf.point([lngCenter, latCenter]);
...
//    if (west < -180) {
//        console.debug(turf.bbox(turf.lineString([[west + 360, south], [180, north]])));
//        console.debug(turf.bbox(turf.lineString([[-180, south], [east, north]])));
//    } else if (east > 180) {
//        console.debug(turf.bbox(turf.lineString([[west, south], [180, north]])));
//        console.debug(turf.bbox(turf.lineString([[-180, south], [east - 360, north]])));
//    } else {
//        console.debug(turf.bbox(turf.lineString([[west, south], [east, north]])));
//    }
    if (west < -180) {
        retrieve(west + 360, 180, south, north, tilesToFetchDistance);
        retrieve(-180, east, south, north, tilesToFetchDistance);
    } else if (east > 180) {
        retrieve(west, 180, south, north, tilesToFetchDistance);
        retrieve(-180, east - 360, south, north, tilesToFetchDistance);
    } else {
        retrieve(west, east, south, north, tilesToFetchDistance);
    }
